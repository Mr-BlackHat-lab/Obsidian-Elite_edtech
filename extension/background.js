...
/* harmony export */   relayQuizToActiveTab: () => (/* binding */ relayQuizToActiveTab)
/* harmony export */ });
// ============================================================
...
let captureAttemptedSessionId = null;
...
function isQuestion(value) {
    ...
    return (typeof candidate.question_id === "string" &&
...
async function ensureAudioCapture(sessionId) {
    if (captureSessionId === sessionId && mediaRecorder?.state === "recording")
        return;
    if (captureAttemptedSessionId === sessionId)
        return;
...
function parseShowQuizMessage(rawData, fallbackSessionId) {
    ...
    try {
        ...
        const sessionCandidate = typeof payload.session_id === "string"
            ...
            : typeof payload.data?.session_id === "string"
                ...
                : fallbackSessionId;
        if (!isQuestion(questionCandidate))
            return null;
        return {
            type: "SHOW_QUIZ",
...
function scheduleReconnect(sessionId, videoUrl) {
    clearReconnectTimer();
    const delay = Math.min(RECONNECT_BASE_MS * Math.max(1, 2 ** reconnectAttempt), RECONNECT_MAX_MS);
    reconnectAttempt += 1;
    reconnectTimer = setTimeout(() => {
        void connectWebSocket(sessionId, videoUrl);
    }, delay);
}
async function connectWebSocket(sessionId, videoUrl = "") {
    if (liveSocket &&
...
async function syncLivePipeline() {
    ...
    const videoUrl = typeof stored.sessionVideoUrl === "string" ? stored.sessionVideoUrl : "";
    ...
    const scorePercent = typeof stored.sessionScore === "number"
 ...
 */
function handleBadgeUpdate(status, scorePercent) {
    if (status === "ACTIVE") {
        const text = scorePercent !== undefined && scorePercent > 0
            ? `${scorePercent}%`
            : "ON";
 ...
 */
async function relayQuizToActiveTab(message) {
    try {
